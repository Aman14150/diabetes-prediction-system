...
def export_csv():
    ...
    # Remove 'id' column index
    id_index = columns.index('id')
    columns.pop(id_index)  # remove id

    output = io.StringIO()
    writer = csv.writer(output)

    # Add custom Sr. No. column
    writer.writerow(['Sr. No.'] + columns)

    for i, row in enumerate(rows, start=1):
        row_list = list(row)
        row_list.pop(id_index)  # remove id from row
        writer.writerow([i] + row_list)  # add serial number
...
def predict():
    try:
        ...
        # --- Prediction (after scaled_input computed) ---
probs = None
pred_label = None

# If model supports predict_proba
if hasattr(model, "predict_proba"):
    probs = model.predict_proba(scaled_input)[0].tolist()  # [prob_class0, prob_class1]
    pred_idx = int(model.predict(scaled_input)[0])         # 0 or 1
    pred_label = "Diabetic" if pred_idx == 1 else "Non-Diabetic"
    confidence = round(probs[pred_idx] * 100, 2)          # e.g. 87.23 (percent)
else:
...
cursor.execute(sql, (name, gender, data[7], data[0], data[1], data[2], data[3], data[4], data[5], data[6], pred_label))
db.commit()

# Feature importance (as earlier)
feature_importance = None
if hasattr(model, 'feature_importances_'):
    feature_importance = dict(zip(fields, model.feature_importances_.tolist()))

return jsonify({
    'status': 'success',
    'result': pred_label,
    'confidence': confidence,               # single number (percent) or null
    'probabilities': probs,                 # list like [0.12, 0.88]
