document.addEventListener('DOMContentLoaded', function () {
    ...
    function showConfidence(conf) {
        if (!confidenceWrapper || conf === null || conf === undefined) return;

        confidenceWrapper.style.display = 'block';
        confidenceText.textContent = `Accuracy Score: ${conf}%`;
        confidenceBar.style.width = conf + '%';

        if (conf >= 80) confidenceBar.style.background = '#10b981';
        else if (conf >= 60) confidenceBar.style.background = '#f59e0b';
        else confidenceBar.style.background = '#ef4444';
    }
    ...
    if (closePredictionBtn) {
        closePredictionBtn.addEventListener('click', function () {
            ...
            if (confidenceWrapper) confidenceWrapper.style.display = 'none';
        });
    }
        ...
        form.addEventListener('submit', function (e) {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    console.log("✅ Prediction Success:", data.result, "Accuracy Score:", data.confidence);
